...
Every Component declares its Metadata once — description, purpose collections, when to use it, when another route serves better, terminal stance, and accessibility notes — and codegen projects that authority into the dedicated React-free `./components` entrypoint. `componentMetadata` is the canonical array in Catalogue order. `componentAuthorGuide` is the Markdown guide generated from it: an index of the purpose collections, then one section per Component beneath its Group naming the React adapter, its props type, and the terminal renderer the generated barrels publish, its use and refusal guidance, its accessibility contract, its canonical examples, and its documented props — the props declaration, every property with its type and doc, and each package type the properties reference — with an explicit line wherever optional Metadata is absent. `packageVersion` names the release the guide describes, and `cataloguePurposeDetails` carries the label and scope of each purpose collection.
...
The guide exists so a coding agent selects and authors Components from the package it is building against rather than from memory. [`skills/use-discern-design-system/`](skills/use-discern-design-system/) is the matching review copy in this repository: its script prints the installed package's guide whole or filtered by purpose, Group, or slug, prints the diagram and chart kind guides, and reports the resolved specifier, version, and resolution route; the playbook walks selection, the pinned-imagery check, the document contract, public-contract authoring, diagram and chart checking, and verification. Its eventual distribution is intentionally undecided and the skill is not part of the published package.
