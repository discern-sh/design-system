...
- [ ] **Resolve every pending Component CLI stance.** Add a pure renderer or a reasoned exemption for each of the 27 remaining pending Metadata entries and lower `[standards.cli_pending]` with each landed tranche; at zero, make pending an always-on conformance error and retire the standard. Evidence: `src/generated/cli-registry.ts`, `discern.toml`.
- [ ] **Implement the interactive terminal driver.** Produce the visual states in `src/cli/interactive-states.ts` from raw-mode input, validation, timing, and frame replacement without moving effects into renderers. Evidence: `src/cli/interactive-states.ts` contains states and `src/cli/` contains no `interactive/` driver.
