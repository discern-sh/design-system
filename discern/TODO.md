...
- [ ] **Copy affordances render inert in static consumer output.** Result summary, Command, Path reference, and Diagnostic compose Copy button, whose clipboard write lives in a React `onClick` handler; under `renderToStaticMarkup` the handler is dropped, the selection emits no `discern.js` because no Component declares browser behavior, and the consumer page ships buttons that do nothing. The generated author guide calls the action "adapter-only" without saying it needs live React. Decide whether the copy affordance moves behind declared browser behavior (so the runtime emits it), becomes a consumer opt-in prop that static consumers leave off, or stays as is with the Metadata accessibility line stating that static rendering drops it. Evidence: `src/components/docs/copy-button/copy-button.tsx:47`; `src/components/workflow/result-summary/result-summary.tsx` (`machineReadable`); `skills/use-discern-design-system/SKILL.md` (React step: static HTML, no hydration).
...
- [ ] **Decide how consumers receive the `use-discern-design-system` skill.** The skill lives under `skills/` and installs by linking or copying the directory; it is not in the JSR publish set, and discern does not bundle it. Options: publish it in the package artifact, have the discern product bundle it for every discern project, or keep the link-and-copy route documented in the README. Evidence: `skills/use-discern-design-system/SKILL.md`; `deno.json` `publish.include`; `README.md` (Component Metadata and author guide).
- [ ] **Glyph Atlas: build the Catalogue search and detail projection from the private authority.** Iterate `glyphAtlasData.canonical`, search its derived `searchTerms`, join `glyphAtlasData.aliases` by `canonicalId`, normalize authored queries through `normalizeGlyphSearchTerm()`, and expose provenance, presentation, measured width, hazards, recommendations, and the accessibility disclaimer without creating a second registry. Evidence: `src/glyphs/atlas.ts`; `src/glyphs/validation.ts`; `map/15-glyphs/README.md`.
- [ ] **Discern Glyphs: review names before creating a public package contract.** Product-review the curated machine names, categories, recommendation states, surface guidance, and ASCII degradations; then design a documented `./glyphs` export with SemVer and changelog coverage. Treat any migration of Icon, motif, triangle, or chart-ramp consumers as separate compatibility work rather than making publication imply consolidation. Evidence: `src/glyphs/atlas.ts`; `map/_adr/0042-separate-unicode-glyph-identity-from-curated-aliases.md`.
