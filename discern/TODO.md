...
- [ ] **Evaluate compact chart reflow separately from terminal application foundations.** Decide spatial reflow, compact chart presentation, and exhaustive source-value annotations before changing mini-charts or DataFigure. The application regions can host existing blocks; this does not establish a chart redesign requirement.
...
- [ ] **Author use-when and refusal guidance for the forty-nine Components still carrying the absence line.** Core, Forms, Feedback, Navigation, People, Agents, Docs, Marketing, Cluster, and Diffstat state only a description, so the guide's promise to select by the reader's task holds for the editorial shortlist but not the rest. Write each use-when as a situation an author recognises and route every refusal to the sibling that serves instead; the first of each becomes an eval prompt. Evidence: `deno eval 'import { componentMetadata } from "./src/component-metadata.ts"; console.log(componentMetadata.filter((m) => !m.useWhen?.length).map((m) => m.slug))'`; `discern/skills/add-a-component/SKILL.md` (the authoring brief).
- [ ] **Decide whether Theme toggle gets a package-owned static posture.** Static consumers re-implement the same click handler over the `data-discern-*-label` and `-glyph` attributes the markup already carries; the skill documents that recipe. The alternative is an uncontrolled posture whose `theme-toggle` behavior in `discern.js` flips the nearest `data-discern-root`'s theme, which contradicts the Metadata line that the Component never mutates the document. Needs an owner decision (and an ADR if the stance changes). Evidence: `src/components/core/theme-toggle/theme-toggle.meta.ts` (accessibility); `skills/use-discern-design-system/SKILL.md` (controlled adapters in static output).
- [ ] **Offer a `fonts-inline` runtime asset selection for single-file output.** `assets: ["fonts"]` emits `fonts.css` with `url("./fonts/…")` references, so a self-contained HTML deliverable rewrites them to `data:` URIs by hand (about 180 KB). An inline selection would emit the same faces embedded; the `font_transfer` standard would need a matching measure. Evidence: `src/runtime-assets.ts`; `assets/fonts.css`; `discern.toml` `[standards.font_transfer]`.
- [ ] **Offer cheaper canonical imagery for agents.** Example images are captured in a 960-pixel harness at 2× density and several are mostly empty canvas, so each costs an agent about a thousand tokens to inspect. A half-scale variant or a per-Group contact sheet beside the canonical PNGs would make the imagery step cheaper without changing the review artifacts. Evidence: `catalogue/example-images/contract.ts` (viewport and harness); `scripts/component-example-images.ts`.
- [ ] **Widen flow edge-label placement beyond two candidates per segment.** About five percent of dense generated flows refuse with "no deterministic clear placement" because a decision's branch labels find no clear spot beside their short runs. More candidates along the longest run, or a label lane between ranks, would place them; the fuzz generator in `tests/diagram/flow_fuzz_test.ts` measures the rate. Evidence: `src/diagram/kinds/flow/flow.layout.ts` (`labelCandidates`, `placeEdgeLabels`).
