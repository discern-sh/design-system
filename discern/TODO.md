...
- [ ] **One example can change how the renderer rasterizes every later capture on the same page.** Capturing `masonry--default--light` (plan index 50) permanently shifts text rasterization for the rest of that page's life: `paragraph`, `heading`, `code-listing`, and `table` then move by up to 1,850 pixels at a channel delta of 183 — deterministic, plainly visible, and far outside the raster tolerance. Navigating to a new document does not clear it; only a new page does, which is why every capture still pays for `context.newPage()`. Worth understanding rather than routing around: the same mechanism is a plausible source of the smaller raster noise the writer now absorbs, and identifying it may retire both problems. Start by finding what masonry does that outlives its document — a compositor or GPU-rasterization mode switch is the leading suspect, and `browserArguments` already carries `--disable-gpu`. Evidence: `scripts/component-example-images.ts` (`withIsolatedCapturePage`); `src/components/layout/masonry/`; `catalogue/example-images/contract.ts` (`browserArguments`).

## 🟠 Cleanup — known dead or slow code

- [ ] **Opening a capture document costs most of each image's time.** A full run is about 944s for 682 images. Rendering is not the cost: `context.newPage()` measures 700–800ms of the ~1300ms per image, against 220ms of navigation, 74ms of readiness, and 35ms for the screenshot. The two obvious levers are already ruled out — reusing a page (~272ms per image) and four concurrent pages (~83ms) are both **unsafe** while the rasterization-contamination item above stands, and neither may be adopted on speed grounds alone. So the open question is narrow: why does opening a page cost most of a second here, and can it be made cheap without sharing renderer state? Worth checking `--disable-gpu` and the route interceptor's per-page setup, and whether a pool of pre-opened but never-reused pages hides the latency. Evidence: `scripts/component-example-images.ts` (`startCaptureBrowser`, `withIsolatedCapturePage`); `catalogue/example-images/contract.ts` (`browserArguments`).
- [ ] **Any capture-input edit recaptures the whole corpus.** `componentExampleCaptureSourceHash` hashes whole files, so touching `deno.json`, `package.json`, or the capture script itself restages all 682 images even though nothing they render changed — the reason a version bump costs a 16-minute recapture. Recapturing only the entries whose own inputs moved would make a bump nearly free. The current over-broad hash is deliberate, so any scoping must fail towards recapturing more: a per-Component input set that silently missed a shared token or foundation edit would commit stale evidence. Evidence: `scripts/component-example-images.ts` (`CAPTURE_INPUTS`, `componentExampleCaptureSourceHash`); `map/80-development/done-gate-gotchas.md`.
