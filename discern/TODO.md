...
- [ ] **Discern Glyphs: implement the accepted public package boundary.** Design and publish the React-free `./glyphs` projection of approved candidate aliases and resolution metadata with SemVer and changelog coverage, while keeping the canonical Atlas private. Treat any migration of Icon, motif, triangle, or chart-ramp consumers as separate compatibility work rather than making publication imply consolidation. Evidence: `src/glyphs/atlas.ts`; `map/_adr/0044-publish-curated-glyphs-not-canonical-atlas.md`.
