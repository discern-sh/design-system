...
- [ ] **Component example recapture is slow enough to distort ordinary work.** `deno task catalogue:images --update` walks the whole corpus in a real browser. One measured full run: 944.35s total for 682 images, of which 941.83s is capture (1381ms/image) against a 1.44s build and a 0.48s browser launch — so essentially all of the cost is serial per-image capture, and neither the build nor browser startup is worth optimising. The verify path additionally captures each witness twice for a geometry check, and it lands inside the gate's test stage on every release. Investigate before changing anything: capture concurrency across pages or contexts, what makes a single capture cost over a second, and recapturing only the entries whose own inputs moved rather than the whole corpus on any source-hash change. Any per-entry input scoping must stay conservative — the current whole-file hash is deliberately over-broad. Evidence: `scripts/component-example-images.ts` (`CAPTURE_INPUTS`, `verifyImages`, `withCaptureServer`); `deno.json` `catalogue:images` and `test` tasks.
