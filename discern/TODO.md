...
- [ ] **Cure the remaining scrollable text regions without keyboard access.** Terminal and Code listing now follow Chart's focusable-viewport pattern (`role="group"`, derived `aria-label`, `tabIndex={0}`, `:focus-visible` outline) after axe flagged `scrollable-region-focusable` on the landing page. The same class survives wherever a text-content surface scrolls with no focusable children: `code-block` (root `overflow: auto`), `table` (wrapper `overflow-x: auto`), and the workflow pre bodies in `raw-output`, `command`, `expected-result`, and `diagnostic`; `diagram` should be checked against Chart's cure. Interactive-content scrollers (search-palette, tabs, dialog, hover-card, site-header nav, breadcrumbs) pass the rule through their focusable children and need nothing. Finish with a structural test that fails any component CSS declaring `overflow(-x/-y): auto|scroll` on a content body whose markup lacks a focusable scroll container, so future members enrol automatically. Evidence: `grep -rn 'overflow[^:]*: *\(auto\|scroll\)' src/components/*/*/*.css`; scripts/conformance.ts landing axe scan.
