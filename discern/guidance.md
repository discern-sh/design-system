...
_(One-line pitch: what discern-design-system is and who it's for — replace this
line.)_

## Conventions

_(Replace this section.)_ Language idioms, style, structure, naming, error
handling — anything the tooling enforces. Keep it aligned with the
`[capabilities]` you wire in `discern.toml`, so the written rule and the
enforced rule never disagree.
