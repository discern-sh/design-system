...
4. `discern prepare` while iterating; `discern done` before calling it done. The Catalogue build type-checks every bounded example, and every Web-capable entry auto-enrols in light and dark accessibility scans, generated imagery, and settled review. Add `export const conformance = [...]` scenarios (see `catalogue/conformance.ts`) when the component has keyboard or focus behaviour worth pinning.
