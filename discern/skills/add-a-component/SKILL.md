...
- **Start on the monochrome field.** Component CSS consumes public roles and axes; it never contains `light-dark()` or any other Theme branch. Primary actions pair `--discern-color-action` with `--discern-color-on-action`; do not put text over `accent-400` or a louder rung with an independently chosen ink. Use the field's derived role when a treatment must change with polarity.
...
- **Own painted backdrops.** The element that paints a floating or overlay surface declares the exact opaque role it consumes as `data-discern-floating-surface="surface"`, `"canvas"`, or `"inverse-surface"`. Behaviour panels also retain the shared floating root/trigger/panel attributes. A wash or sunken role cannot own a backdrop.
...
4. `discern prepare` while iterating; `discern done` before calling it done. The Catalogue build type-checks every bounded example, and every Web-capable entry auto-enrols in light and dark accessibility scans, generated imagery, settled review, field-axis target/type/focus checks, opaque floating-surface checks, and status-witness checks. Add `export const conformance = [...]` scenarios (see `catalogue/conformance.ts`) when the component has keyboard or focus behaviour worth pinning.
