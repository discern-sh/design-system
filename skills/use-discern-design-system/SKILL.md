...
- **Controlled adapters in static output**: `ThemeToggle` requires `onThemeChange` and `Tag` accepts `onRemove`; neither handler survives static rendering. Render `ThemeToggle` with the resolved theme and a no-op handler, then wire the consumer's own script to the attributes the markup carries: on click, flip `data-discern-theme` on the root, persist the choice, and swap the button's `aria-label` and glyph text from its `data-discern-to-light-label`, `data-discern-to-dark-label`, `data-discern-light-glyph`, and `data-discern-dark-glyph` attributes. Omit `onRemove` from a static `Tag`; it then renders no remove button.
