...
function componentAuthorGuideSource(
  ...
  documented?: DocumentedSymbolIndex,
  ...
  return documented === undefined ? source : {
    ...source,
    props: renderComponentPropsGuide(
      documented,
      componentReactExportName(component.meta.slug),
    ),
  };
}

/**
 * Document every Component's React surface once with `deno doc`, private
 * declarations included, so the guide can state file-local helper types
 * that the props reference. The implementation and its `mod.ts` are both
 * entrypoints: the first carries private helpers, the second the shared
 * vocabulary it re-exports.
 */
async function documentComponentSurfaces(
  components: readonly ComponentSource[],
): Promise<DocumentedSymbolIndex> {
  const entrypoints = components.flatMap((component) => [
    component.implementationUrl.href,
    new URL("mod.ts", component.metaUrl).href,
  ]);
  ...
  if (result.code !== 0) {
    throw new Error(
      `deno doc could not document the Component React surface:\n${
        new TextDecoder().decode(result.stderr)
      }`,
    );
  }
  return new DocumentedSymbolIndex(
    JSON.parse(new TextDecoder().decode(result.stdout)) as DocumentedGraph,
  );
}

async function generateComponentAuthorEvals(): Promise<string> {
  const components = await loadComponentSources();
  return renderComponentAuthorEvals(
    components.map((component) => componentAuthorGuideSource(component)),
  );
}
...
async function generateComponentMetadata(): Promise<string> {
  ...
  const documented = await documentComponentSurfaces(components);
  const guide = renderComponentAuthorGuide(
    components.map((component) =>
      componentAuthorGuideSource(component, documented)
    ),
  );
