...
async function generateRegistry(
  ...
  for (const [index, source] of sources.entries()) {
    ...
    imageEntries.push(
      `  { slug: meta${index}.slug, webExamples: resolveCatalogueWebExamples(meta${index}, componentExampleVocabulary${index}, examples${index}, ${
    ...
    entries.push(
      `  { meta: meta${index}, canonicalExamples: resolveComponentExampleVocabulary(meta${index}, componentExampleVocabulary${index}), webExamples: resolveCatalogueWebExamples(meta${index}, componentExampleVocabulary${index}, examples${index}, ${
        JSON.stringify(source.examplesUrl.pathname)
...
import { resolveCatalogueWebExamples } from "../web-examples.ts";
${imports.join("\n")}
...
function posturesFrom(
  ...
  return resolveComponentReviewPostures(
    meta,
    vocabulary,
    resolveCatalogueWebExamples(meta, vocabulary, module, source),
...
`;
  const imageRegistry = `/* Generated by scripts/build.ts. Do not edit. */
...
/** Resolve one bounded Web renderer without importing any CLI implementation. */
export function catalogueWebExample(
  slug: string,
  id: string,
): CatalogueExample {
...
`;
  ...
  await Deno.writeTextFile(
    new URL("example-image-registry.ts", GENERATED_ROOT),
    imageRegistry,
  );
}
...
/** Project the authored page and layout files into copyable consumer sources. */
export async function writeSignatureCompositionSources(): Promise<void> {
  const componentSource = await Deno.readTextFile(
    new URL(
      "../catalogue/compositions/signature-specimens.tsx",
      import.meta.url,
    ),
  );
  const adapterImport = '"../../src/react.ts"';
  if (componentSource.split(adapterImport).length !== 2) {
    throw new Error(
      "Signature specimens must have exactly one public adapter import",
    );
  }
  const files = [{
  ...
  await Deno.mkdir(GENERATED_ROOT, { recursive: true });
  await Deno.writeTextFile(
    new URL("signature-composition-sources.ts", GENERATED_ROOT),
    `/* Generated by scripts/build.ts. Do not edit. */\nexport const signatureCompositionFiles = ${
...
export async function buildDesignSystem(
  ...
  await progressStep(
    progress,
    "Project composition source files",
    () => writeSignatureCompositionSources(),
  );
  await progressStep(
    progress,
