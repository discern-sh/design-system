...
async function generateRegistry(
  ...
  for (const [index, source] of sources.entries()) {
    ...
    entries.push(
      `  { meta: meta${index}, Examples: examples${index}.default, conformance: scenariosFrom(examples${index}, ${
        JSON.stringify(source.examplesUrl.pathname)
      }) },`,
    );
  }
  const registry =
    `/* Generated by scripts/build.ts. Do not edit. */\nimport type { ComponentType } from "react";\nimport type { ComponentMeta } from "../../src/types/component-meta.ts";\nimport type { ConformanceScenario } from "../conformance.ts";\n${
      imports.join("\n")
    }\n\nfunction scenariosFrom(module: object, source: string): readonly ConformanceScenario[] {\n  const scenarios = "conformance" in module ? module.conformance : [];\n  if (!Array.isArray(scenarios)) throw new TypeError(\`\${source} conformance export must be an array\`);\n  return scenarios as readonly ConformanceScenario[];\n}\n\nexport interface RegistryEntry { readonly meta: ComponentMeta; readonly Examples: ComponentType; readonly conformance: readonly ConformanceScenario[]; }\nexport const registry: readonly RegistryEntry[] = [\n${
      entries.join("\n")
    }\n];\n`;
