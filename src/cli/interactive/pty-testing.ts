/** Optional real-PTY fixture driver adapted from discern's generic script(1) transport. @module */
...
export interface PtyInputPhase {
  ...
  /** Ordered input after readiness; an empty list records an observation-only phase. */
  readonly steps: readonly PtyInputStep[];
}
...
export async function runPtyProcess(
  ...
  const waitForOutput = async (
    ...
    while (
      ...
      if (outputFinished) {
        throw new Error(
          `pseudo-terminal command exited before rendering input markers ${
  ...
  const waitForOutputCondition = async (
    ...
    while (!condition.test(observedOutput(cursor))) {
      if (outputFinished) {
        throw new Error(
          `pseudo-terminal command exited before satisfying keyframe condition ${
            JSON.stringify(condition.description)
          }`,
        );
      }
      await new Promise<void>((resolve) => outputWaiters.push(resolve));
  ...
  if (inputError !== undefined) {
    const message = inputError instanceof Error
      ...
      : String(inputError);
    throw new Error(
      `${message}; ${Deno.build.os} script transport:\n${stdout}${stderr}`,
      { cause: inputError },
    );
  }
  return {
...
function validateInput(
  ...
  for (const phase of phases) {
    ...
    if (!isOutputCondition(phase.waitFor)) {
      assertOutputMarkers(
        typeof phase.waitFor === "string" ? [phase.waitFor] : phase.waitFor,
        "PTY input readiness",
      );
    }
    const capture = phase.capture;
    if (capture !== undefined) {
      ...
      if (capture.when.description.length === 0) {
        throw new TypeError(
          "PTY keyframe readiness description must not be empty",
        );
      }
      keyframeNames.add(capture.name);
    }
    for (const step of phase.steps) {
      if (!Number.isFinite(step.delayMs ?? 0) || (step.delayMs ?? 0) < 0) {
        throw new TypeError("PTY input delay must be non-negative and finite");
      }
      const bytes = inputBytes(step);
