...
const meta = {
  ...
  behaviors: ["search-palette"],
  description:
    "Modal command-palette search on the native dialog element, with a search field, results region, and hint row, controlled by React or rendered as static markup the emitted behaviour opens and dismisses.",
  ...
  useWhen: [
    ...
    "Static HTML needs the complete dialog without hydration: omitting `onOpenChange` renders it closed with `data-discern-search-palette` on it, `data-discern-search-palette-input` on the field, and `data-discern-search-palette-close` on the close control. The selected `search-palette` behaviour opens it from a control carrying `data-discern-search-palette-open` and owns dismissal and focus, while the consumer's own script answers its open and close events with the query, results, and selection.",
  ],
  notWhen: [
    "Use Dialog for a confirmation or form; the palette's field and results region assume a query.",
  ],
  accessibility: [
    "The native dialog provides focus containment, Escape dismissal, and focus return to the opening control; in static mode the search-palette behaviour focuses the field, returns focus, and, for a reader without showModal(), makes the rest of the page inert, wraps Tab, and closes on Escape.",
