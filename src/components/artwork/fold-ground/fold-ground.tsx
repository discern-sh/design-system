...
export const FoldGround: DiscernComponent<HTMLDivElement, FoldGroundProps> =
  forwardRef<HTMLDivElement, FoldGroundProps>(function FoldGround(
    { className, ...props },
    ref,
  ) {
    return (
      <Ground
        ref={ref}
        className={classNames("discern-fold-ground", className)}
        {...props}
      >
        <svg
        ...
        >
          {FOLD_BANDS.map((band, bandIndex) => (
            <g
              key={bandIndex}
              className="discern-fold-ground__band"
              style={{
                "--discern-fold-ground-offset": `${band.offset}s`,
              } as CSSProperties}
            >
              {band.facets.map((facet, facetIndex) => (
                <polygon
                  ...
                  className={facet.accent
                    ? "discern-fold-ground__facet discern-fold-ground__facet--accent"
                    : "discern-fold-ground__facet"}
                  style={{
                    "--discern-fold-ground-facet": facet.tint,
                  } as CSSProperties}
                  points={facet.points}
