...
type CompressionPlateStyle = CSSProperties & {
  readonly "--discern-compression-backdrop-open-x": number;
  readonly "--discern-compression-backdrop-open-y": number;
  readonly "--discern-compression-backdrop-open-slide": string;
  readonly "--discern-compression-backdrop-closed-x": number;
  readonly "--discern-compression-backdrop-closed-y": number;
  readonly "--discern-compression-backdrop-closed-slide": string;
};

const COMPRESSION_PLATE_STYLE: CompressionPlateStyle = {
  "--discern-compression-backdrop-open-x": COMPRESSION_BREATH.open.scaleX,
  "--discern-compression-backdrop-open-y": COMPRESSION_BREATH.open.scaleY,
  "--discern-compression-backdrop-open-slide":
    `${COMPRESSION_BREATH.open.slide}px`,
  "--discern-compression-backdrop-closed-x": COMPRESSION_BREATH.closed.scaleX,
  "--discern-compression-backdrop-closed-y": COMPRESSION_BREATH.closed.scaleY,
  "--discern-compression-backdrop-closed-slide":
...
function flatRun(
  ...
  return (
    <g className={className}>
      <line
        className="discern-compression-backdrop__run"
        x1={outer}
        y1={axis.y}
        x2={junction}
        y2={axis.y}
        ...
        return (
          <line
            ...
            className={tick.major
              ? "discern-compression-backdrop__tick discern-compression-backdrop__tick--major"
              : "discern-compression-backdrop__tick"}
            x1={tick.x}
...
/** Props for the {@linkcode CompressionBackdrop} component. */
export interface CompressionBackdropProps
 ...
 */
export const CompressionBackdrop: DiscernComponent<
  HTMLDivElement,
  CompressionBackdropProps
> = forwardRef<HTMLDivElement, CompressionBackdropProps>(
  function CompressionBackdrop(
    { className, ...props },
    ref,
  ) {
    return (
      <Backdrop
        ref={ref}
        className={classNames("discern-compression-backdrop", className)}
        {...props}
      >
        <svg
        ...
        >
          <g aria-hidden="true">
            {flatRun(
              "discern-compression-backdrop__head",
              HEAD_OUTER,
              COMPRESSION_START,
              COMPRESSION_HEAD_TICKS,
            )}
            {flatRun(
              "discern-compression-backdrop__tail",
              TAIL_OUTER,
              COMPRESSION_END,
              COMPRESSION_TAIL_TICKS,
            )}

            <g className="discern-compression-backdrop__fold">
              <path
                className="discern-compression-backdrop__pleats"
                d={COMPRESSION_PLEATS}
                vectorEffect="non-scaling-stroke"
              />
              <g className="discern-compression-backdrop__creases">
