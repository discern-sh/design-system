...
const HARMONIC_GEOMETRY = Object.freeze({
   ...
   * rendered plane, so the figure follows the Backdrop rather than occupying an
   * inset square.
   */
...
interface GrainStyle extends CSSProperties {
  readonly "--discern-harmonic-backdrop-sx": string;
  readonly "--discern-harmonic-backdrop-sy": string;
  readonly "--discern-harmonic-backdrop-bx": string;
  readonly "--discern-harmonic-backdrop-by": string;
}

interface TrembleStyle extends CSSProperties {
  readonly "--discern-harmonic-backdrop-tx": string;
  readonly "--discern-harmonic-backdrop-ty": string;
}

function grainStyle(grain: HarmonicGrain): GrainStyle {
  return {
    animationDelay: grain.delay,
    "--discern-harmonic-backdrop-sx": `${grain.scattered.x}px`,
    "--discern-harmonic-backdrop-sy": `${grain.scattered.y}px`,
    "--discern-harmonic-backdrop-bx": `${grain.gathered.x}px`,
    "--discern-harmonic-backdrop-by": `${grain.gathered.y}px`,
  };
}
...
/** Props for the {@linkcode HarmonicBackdrop} component. */
export interface HarmonicBackdropProps
 ...
 */
export const HarmonicBackdrop: DiscernComponent<
  HTMLDivElement,
  HarmonicBackdropProps
> = forwardRef<HTMLDivElement, HarmonicBackdropProps>(function HarmonicBackdrop(
  { className, ...props },
  ref,
) {
  return (
    <Backdrop
      ref={ref}
      className={classNames("discern-harmonic-backdrop", className)}
      {...props}
    >
      <svg
      ...
      >
        <g aria-hidden="true">
          <g className="discern-harmonic-backdrop__field">
            {HARMONIC_TREMBLES.map((tremble, phase) => (
              <g
                key={phase}
                className="discern-harmonic-backdrop__tremble"
              ...
              >
                {HARMONIC_GRAINS.filter((grain) => grain.phase === phase).map((
                  ...
                  <circle
                    key={`${grain.x},${grain.y}`}
                    className="discern-harmonic-backdrop__grain"
                    cx={grain.x}
                    cy={grain.y}
                    r={grain.radius}
          ...
          <circle
            className="discern-harmonic-backdrop__driver"
            cx={plate.width / 2}
            cy={plate.height / 2}
            r={2.1}
          />
