/**
 * Report every finding for one diagram spec in a single pass.
 *
 * Rendering stops at the first refusal, which suits a build but not an
 * author iterating on labels: each attempt reveals one more over-long
 * annotation. {@linkcode checkDiagram} runs the same validation, layout,
 * and conformance the projections use, records every budget refusal
 * instead of stopping at the first, and returns them with the one
 * structural or layout refusal, if any, that did end the run.
 *
 * @module
 */
...
function finding(error: DiagramValidationError): DiagramFinding {
  const base = {
    code: error.code,
 ...
 * refusal ends the run and is reported last. Anything that is not a
 * diagram refusal is rethrown unchanged.
 */
export function checkDiagram(spec: unknown): DiagramCheckResult {
  const collected = collectKindFindings(() => prepareDiagram(spec));
  const findings = collected.findings.filter((
    error,
  ): error is DiagramValidationError => error instanceof DiagramValidationError)
    .map(finding);
  if (collected.failure !== undefined) {
    if (!(collected.failure instanceof DiagramValidationError)) {
