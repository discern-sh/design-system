...
- **Preview visible changes before the final gate.** For Component or Catalogue changes, provide the Catalogue URL on the worktree's deterministic port (`discern identity --port`). Link CLI renderer changes with `?surface=cli#component-<slug>` and exercise the CLI playground for interactive Adapter changes. Complete the required visual review while the preview runs. Then stop this effort's preview/watch processes, run `discern done` on the clean final HEAD, and keep the processes stopped through `discern accept`; a landing may remove the worktree. A task grant recorded at the desk covers later green commits this agent submits for the same task, and `discern accept` checks that authority at the landing boundary. See [checkout convergence](map/80-development/checkout-convergence.md).
