...
- **Preview visible changes during review.** For Component or Catalogue changes, provide the Catalogue URL on the worktree's deterministic port (`discern identity --port`). Link CLI renderer changes with `?surface=cli#component-<slug>` and exercise the CLI playground for interactive Adapter changes. While a preview or watcher remains running, retain the checkout with `discern done --retain-checkout`. After the owner approves, stop this effort's preview/watch processes, run `discern done` to release the checkout, then follow its acceptance action. No preview needs to remain running after acceptance. See [validation workspaces](map/80-development/validation-workspaces.md) for the return contract.
