...
Deno.test("PageUp and PageDown jump the visible window of grouped choice machines", async () => {
  const io = new FakeTerminal(["\x1b[6~", "\x1b[6~", "\x1b[5~", "\r"], {
    columns: 42,
    rows: 24,
  });
  assertEquals(
  ...
  const multi = new FakeTerminal(["\x1b[6~", " ", "\r"], {
    columns: 42,
    rows: 24,
  });
  assertEquals(
  ...
  const clamped = new FakeTerminal(["\x1b[F", "\x1b[6~", "\r"], {
    columns: 42,
    rows: 24,
  });
  assertEquals(
...
Deno.test("paging jumps follow the fitted window rather than the requested count", async () => {
  const io = new FakeTerminal(["\x1b[6~", "\x1b[6~", "\r"], {
    columns: 42,
    rows: 8,
  });
  assertEquals(
