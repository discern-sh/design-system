...
function markdownCase(spec: DiagramSpec): {
  readonly source: string;
  readonly resource: MarkdownDiagramResource;
} {
  const diagramSource = `assets/${spec.kind}-reference.svg`;
  return {
    source: `![${diagramAltText(spec)}](${diagramSource} "${spec.summary}")`,
    resource: { source: diagramSource, spec },
  };
}

Deno.test("every generated kind traverses every public projection from one spec", () => {
  assertEquals(
    diagramKindRegistry.map(({ meta }) => meta.slug),
    ["flow", "architecture", "cycle", "sequence", "timeline"],
  );
  for (const entry of diagramKindRegistry) {
    const spec = entry.fixtures[0] as DiagramSpec;
    ...
    const markdown = markdownCase(spec);
