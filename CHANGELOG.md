...
- Add the public `renderResultSummaryGroupCli()` collection renderer for widest-prefix alignment across mixed result states, and present command suggestions with stable `Run:` action grammar instead of a leading shell-transcript `$`. Display Heading and Anchor heading now own one leading blank line by default and accept a validated nonnegative `leadingBlankLines` override; the CLI Playground adds direct height, triangle-status, and internal heading-candidate review journeys.
