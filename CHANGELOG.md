...
- Add the Agents group, the eleventh canonical group: six components for representing machine actors alongside the People set — Agent avatar, Agent persona, Agent mention, Worklog, Transcript, and Receipt. The grammar is deliberate: people are warm circles set in serif, agents are dark squares set in mono, so mixed rosters read at a glance.
...
- The identity-tile size scale becomes public tokens — `--discern-avatar-size-xs` through `--discern-avatar-size-xl` — that Avatar and Agent avatar both resolve their steps from, held by a release test so the shared scale cannot drift. Terminal's surface and text now source from `--discern-color-inverse-surface` and `--discern-color-inverse-ink`, so consumer themes move terminals and agent tiles together; in the dark theme the terminal sits on the same inverse layer as the tiles instead of near-black.
