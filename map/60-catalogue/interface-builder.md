...
Workspace and Preview each render the shared [`AppearanceControl`](../../catalogue/shell/appearance.tsx) and consume the same [sanctioned options](../../catalogue/shell/appearance-options.ts) or one bounded field point. Their state is independent, but their semantics are not forked: invalid stored or URL accents and field points canonicalise through the shared authorities. Selecting a named quick choice clears that scope's field point. Generated images follow the Preview Theme. Workspace Appearance never recolours editor-selection facts.
