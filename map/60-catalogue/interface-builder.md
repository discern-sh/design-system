...
Workspace and Preview each render the shared [`AppearanceControl`](../../catalogue/shell/appearance.tsx) and consume the same [sanctioned options](../../catalogue/shell/appearance-options.ts). Their state is independent, but their semantics are not forked: invalid stored or URL accents canonicalise through the shared option authority. Generated images follow the Preview Theme. Workspace Appearance never recolours editor-selection facts.
