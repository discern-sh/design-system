...
The Foundations index leads to three distinct review modes. The Field page at `/catalogue/foundations/field/` shares the global Appearance control's axis implementation, reads the emitted roles back from the browser, and copies the public Root and Appearance scope used by the live page. It consumes the package evaluator and full-domain admission proof rather than maintaining Catalogue-specific role or collision arithmetic. Its browser diagnostic composes real Buttons, semantic Badges, and overlapping AvatarGroups to show Accent 255 inside Field, Field inside Accent 120, and Accent 335 inside Accent 245; the adjacent terminal diagnostic mirrors those directions with real status Components through locally derived public presenters. Both keep semantic content and the inherited non-Appearance coordinates stable. The terminal pole inspector renders the selected Appearance and hue on both honest grounds. Public Input, Select, Button, and Copy button Components supply the controls; no slider exception is needed because Input owns the native range type.
...
Appearance is one orthogonal state boundary: Theme policy, Field or Accent identity, numeric Accent hue, and the four Field axes remain independent. [`shell/appearance-options.ts`](../../catalogue/shell/appearance-options.ts) owns only named numeric hue conveniences; the package owns bounds, labels, polarity, roles, evaluation, and admission. [`shell/field-state.ts`](../../catalogue/shell/field-state.ts) serializes the public field point, [`shell/appearance-state.ts`](../../catalogue/shell/appearance-state.ts) owns canonical parsing and navigation preservation, and [`shell/appearance.tsx`](../../catalogue/shell/appearance.tsx) projects the compact global control. Every routed Catalogue page can switch Field/Accent, enter any hue in the public `0–360` domain, and move Darkness, Structure, Emphasis, or Density without one input erasing another. Named hues set the same number; Blue is the hue-255 convenience.

The canonical URL/storage coordinates are `theme`, `appearance`, `accent`, and `field`, where `accent` is numeric and `field` carries exactly four axes. URL state wins on load, storage supplies the complete prior state when the URL is silent, Back/Forward restores explicit entries, and local navigation merges any coordinates it omits. Existing named `accent=` values and five-part `field=darkness,structure,emphasis,density,preset` links migrate to the canonical representation; malformed or partial coordinates fail closed. System follows the OS at a real pole. Light and Dark snap Darkness to their poles; moving Darkness away from a pole exits System and exposes the polarity-selected native scheme with live-control hysteresis. Live input replaces the current history entry, so sliders keep the URL current without flooding history.

The Component review instrument accepts the same complete state and links both poles, the signed 0A midpoints, Structure and Density extremes, and both appearances. Builder protocol v5 carries an independent Preview state across the frame while the workspace owns a separate state; both use the same control and serialization rules.
...
| Foundations Field       | shared Appearance identity, numeric hue, Darkness, Structure, Emphasis, Density       |
