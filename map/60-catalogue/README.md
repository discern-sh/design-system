...
Diagram's detail route is the visual grammar for the closed kind library. Its Web surface renders minimal, representative, and dense-but-supported architecture, cycle, sequence, and timeline specs beside the established flow cases, plus literal light, dark, and adaptive standalone assets, Markdown promotion, and DataFigure composition. Its CLI surface shows enhanced flow, cycle, and sequence frames; architecture and timeline descriptions; explicit description mode; and narrow or dense typed fallbacks. These are shared kind fixtures or Component examples rather than hand-drawn preview scenes, so codegen and the registry exercise the same facts that the Catalogue displays.
