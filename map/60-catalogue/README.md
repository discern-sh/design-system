...
The Foundations index leads to three distinct review modes. The Field page at `/catalogue/foundations/field/` shares the global Appearance control's axis implementation, reads the emitted roles back from the browser, and copies the public Root and Appearance scope used by the live page. It consumes the package evaluator and full-domain admission proof rather than maintaining Catalogue-specific role or collision arithmetic. Its diagnostic scope sheet composes real Buttons, semantic Badges, and overlapping AvatarGroups to show Accent 255 inside Field, Field inside Accent 120, and Accent 335 inside Accent 245; the nested regions change colour identity while inheriting Structure and Density. The existing CLI pole inspector remains a separate terminal instrument until terminal Appearance integration lands. Public Input, Select, Button, and Copy button Components supply the controls; no slider exception is needed because Input owns the native range type.
...
| Terminal layouts        | preset/custom capabilities and grid, plus the existing shared browser Appearance context |
