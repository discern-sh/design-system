...
[`shell/appearance-options.ts`](../../catalogue/shell/appearance-options.ts) is the reusable project-facing authority for the ordered named review presets, their single default, and the detailed numerical floor loop. Every named option is exhaustively proved in Light and Dark for text contrast, focus contrast on all semantic soft surfaces, semantic separation, and categorical-series visibility and distinction. The Field page calls that same loop at its current point and renders its returned margins and exact failure strings; tests do not own a parallel proof. Unknown named values and malformed field points fail closed. [ADR 0037](../_adr/0037-expose-only-semantically-safe-appearance-presets.md) records why the Catalogue does not present the public low-level hue Token as an unrestricted safe range.
