...
- Looping motion means ongoing work or explicitly ambient Artwork. LightBackdrop starts still; request its ambient motion only on a selected surface and expose a visible still control. The still frame contains the complete meaning, and reduced motion removes movement without removing state.
...
Run `deno task serve`, then open `/catalogue/reviews/components/` on the port from `discern identity --port`. The route is local-only, absent from public navigation, and outside the package export graph. Its URL records Group, Component, canonical example, posture, state category, local width, Theme policy, the optional accent hue, every appearance axis, motion mode, contact sheet or reel, and production or diagnostic timing.

The settled contact sheet defaults to one Group and mounts only the selected population. The reel mounts one item, remounts it on Replay, runs production Token timing unchanged, and scopes the 4× diagnostic timing override to that specimen. Named narrow, medium, and wide widths allocate 390, 720, and 1120 pixels to embedded specimens inside a horizontally contained scroller while the page reports its actual viewport independently. Vertical wheel input over a specimen or its surrounding frame continues to the document; the review wrapper does not own a separate vertical viewport. The canonical Appearance coordinates place the sheet at any bounded axes and hue. Judgment links keep every unrelated coordinate while selecting both poles, the signed 0A `0.25`/`0.5`/`0.75` points, low/high Structure and Density, Monochrome, and Accent 255. An authored posture with an explicit Appearance still wins for its own specimen.
...
The review instrument consumes the same orthogonal Catalogue state as the global header control. The optional accent hue and every axis serialize independently; selecting a named hue changes only the number, and malformed or partial coordinates fail closed. The complete public `0–360` Accent domain is reviewable, with `360` canonicalized to `0`.

The package's evaluator and admission proof own chromatic projection, semantic-family separation, action inversion, and opaque identity surfaces. The Catalogue does not repeat that arithmetic or curate a safe subset. [ADR 0043](../_adr/0043-project-accent-from-the-field.md) records the full-domain and symmetric-scope contract, and [ADR 0045](../_adr/0045-name-the-model-appearance-and-tint-the-pigments.md) the optional accent and pigment tints. The Appearance diagnostic demonstrates monochrome → Accent, Accent → monochrome, and Accent → Accent replacement with real browser and terminal Components while inherited Structure, Density, semantic text, and non-colour witnesses remain visible.

Cross-surface conformance derives its Web and rendered-CLI population from the generated registry. It proves canonical example identity, the exact numeric hue—including unnamed fractional input—honest terminal pole selection, every ANSI depth, no-colour and ASCII witnesses, and a synthetic future renderer that drops Appearance. Numerical chromatic and quantisation proofs remain with the package authorities; this layer guards only the Catalogue-to-renderer route and its visible use sites.

The selected three-purpose material review is available on the same route with `study=signature`. Its visible controls switch complete reading, operational and marketing specimens, theme, local allocation, still/production motion and independently selected treatments. The [voice and material guide](../20-components/material-presence.md) gives its reusable rules; the private planning record retains the bounded direction comparison and owner decisions.
