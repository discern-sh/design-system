...
The canonical `Artwork` Group begins with a shared `Ground` Component and seven dependent Grounds: Survey, Approach, Fold, Aperture, Impression, Envelope, and Cleave. `Ground` owns presence, accent, ambient-or-still motion, containment, and accessibility. Each concrete Ground owns one static React/SVG/CSS composition and depends only on `Ground`, so selecting one includes the common contract without including its siblings. Impression accepts a consumer glyph and defaults to the neutral half-disc `◐`; the package does not make discern's triangle its generic identity.

Hero block accepts a generic decorative `ground` slot but does not import the Artwork Group. Grounds remain useful behind any positioned consumer section, and selecting Hero block alone acquires no artwork bytes.
