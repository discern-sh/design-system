...
A synchronous activation handler may return a foreground operation. Only Enter on an available choice activates it; shortcut handlers change views or exit. The operation runs with terminal ownership restored, and the same navigation state re-enters the bracket afterwards. Background updates replace immutable views through a coalescing mailbox and never block on providers inside the key loop.

Raw-terminal restoration releases pending native input through `TerminalIO.cancelRead` before returning canonical input. `DenoTerminalIO` uses the lazily initialized `node:process` stdin stream, pauses and unreferences it between reads, and leaves stdin open for the foreground operation. The read broker drops only a cancelled pending operation; already-read bytes and protocol filters remain ordered. A timed lone-Escape continuation, cancellation, or provider fault therefore cannot keep the process alive or compete for a child's input. Custom native I/O implementations and forwarding wrappers carry the same release contract; no stream or process launches at module import.
