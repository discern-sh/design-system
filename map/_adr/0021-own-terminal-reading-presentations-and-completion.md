...
Every request selects `completion?: InteractionCompletionPolicy`, whose values are `"retain-frame"` and `"clear-frame"`; omission means `"retain-frame"`. `runInteraction` applies the policy only after successful validation. Retention paints and finishes the submitted frame exactly as before. Clearing asks the active `InlineFramePainter` to erase its current frame while that geometry is authoritative, then returns without painting a submitted record. Validation errors and cancellation keep their established frames. The driver does not expose erase sequences or painter state to consumers, crop a frame, or guess reachable rows.
