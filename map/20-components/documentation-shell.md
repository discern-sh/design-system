...
Selecting Search palette emits the `search-palette` behaviour, which owns opening, dismissal, focus, the hand-off from an open Docs layout drawer, and the fallback for a reader without `showModal()`; the consumer's script keeps the index, ranking, rendering, and selection, and answers the behaviour's open and close events. [Static search palette](../40-runtime-emitter/static-search.md) states the contract.
