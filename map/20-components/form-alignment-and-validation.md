...
Checkbox and Radio share a two-column grid: the indicator column is `--discern-choice-control` wide, and label text plus description share the second column, so descriptions align with the label text at every density and zoom rather than through a fixed indent. The indicator centres against the *first line* of the label (`(1lh − size) / 2`), which keeps multiline labels aligned; the Switch track holds to the first line the same way instead of floating beside the whole text block. Labels span the full row as one generous click target, text wraps anywhere long tokens demand it, and forced-colours mode keeps the selected radio dot and the switch thumb visible with system colours because their fills would otherwise be stripped with the background.
