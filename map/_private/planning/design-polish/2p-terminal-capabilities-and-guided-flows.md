# 2P — Polish terminal previews and guided flows

People can judge terminal Components under real capabilities, inspect a clean presentation, and run a complete guided flow without losing prior input.

**Proposal coverage:** O1, O2, O3, O5. **Worktree name:** `refine-2p`. **Relative size:** L. **Programme:** design-polish.

Other streams are in flight. You own `2P` only; do not launch, dispatch, or supervise the sibling briefs.
...
Once ready, call `discern_start` from the main checkout with the literal name `refine-2p`, then re-root all reads, edits, commands, and discern calls to its returned absolute path. Follow the await tool's start/update hint if applicable. Read the worktree's `AGENTS.md`, `map/00-orientation/design-principles.md`, and this brief there before source work. Verify every anchor against the live tree; 1C intentionally moves some Catalogue anchors.

## Background and outcome

Component CLI previews currently use a fixed 80-column profile, while Terminal lab already has width/height, charset, and colour controls. The interactive adapter supports sequential forms, but each step must manually thread an untyped previous value into its request. O3 and O5 share the same fixtures and therefore belong together.
...
- `src/cli/interactive/sequential-form.ts`
- `src/cli/interactive/basic-requests.ts`
- `src/cli/interactive/choice-requests.ts`
- `scripts/playground/journeys.ts`
- `tests/catalogue_cli_preview_test.ts`
- `tests/catalogue_terminal_lab_state_test.ts`
- `tests/cli/interactive_requests_test.ts`
- `catalogue/terminal-layout-inspector.tsx`

## Deliverables

- **O1.** Reuse a single capability-control/state authority from Terminal lab for per-component CLI inspection: meaningful widths, Unicode/ASCII, colour depths, and any relevant height. Keep the existing deterministic canonical frame profile for conformance/capture; interactive inspection must not silently rewrite example facts or ignore example-specific constraints.

- **O2.** Make the clean terminal frame the default presentation, with rulers, geometry, and diagnostics in an explicit inspect mode. Consume 1A's public SegmentedControl for suitable short, mutually exclusive viewing choices, rather than another private segmented implementation. Preserve the tools and their readable labels. Distinguish actual cell allocation from scaled browser display and keep overflow keyboard-accessible.

- **O5.** Add typed per-kind sequential step constructors that carry previous values into request initial state automatically, while preserving the general closure escape hatch. Derive types from existing request contracts rather than duplicating them. Define precedence for explicit initial values, back-navigation, conditional/skipped steps, and changed upstream choices. Keep effects in the interactive adapter and frames pure.

- **O3.** Demonstrate complete entry→validation correction→back→completion and cancellation flows with the public interactive adapter, reusing the new constructors. Provide honest frame sequences in the Catalogue and a live CLI playground journey; label simulated/replayed frames as such. Include retained values and clear completion/cancel states, not a collection of disconnected snapshots.

## Ownership and exclusions

You own:

- CLI preview and Terminal Catalogue modules/styles/tests, including the cli-preview.css sheet created by 1C.
- Sequential-form typed constructor additions and exact exports from src/cli/interactive/mod.ts, plus related request tests.
- Playground guided-flow journeys/fixtures and the matching CLI compositions; the resolved typed-step TODO entry.

Out of scope: O4 ANSI/plain copy redesign, new request kinds, wide-A/IME work, global ANSI/palette changes, chart or diagram renderer changes, and component-detail layout/prop controls. All proposals deferred to `discern/TODO.md` stay deferred. Do not opportunistically implement a sibling brief.
...
- The same capability state produces the same Component/Terminal-lab frame; invalid parameters normalize, constraints remain honest, and canonical captures remain deterministic.
- Typed tests prove supported step values and invalid combinations; behaviour tests prove previous values survive back/correct/re-enter, skipped/changed steps, completion, and cancellation.
- A live playground journey and Catalogue replay preserve the same semantic frames across supported widths, colour depths, and Unicode/ASCII.
...
- Run the Catalogue at the worktree's deterministic `discern identity --port` port and complete the required visual review at the exact localhost URLs. Stop the preview before the final `discern_done` and keep it stopped through `discern_accept`; a landing may remove the worktree. Preview evidence: A Component CLI URL, Terminal lab clean/inspect URLs, and the exact command/steps for the live guided CLI journey. Include `?surface=cli#component-<slug>` links for changed CLI renderers, and the live playground when an interactive adapter flow changes.
