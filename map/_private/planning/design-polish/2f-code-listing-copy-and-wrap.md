...
- Run the Catalogue at the worktree's deterministic `discern identity --port` port and complete the required visual review at the exact localhost URLs. Stop the preview before the final `discern_done` and keep it stopped through `discern_accept`; a landing may remove the worktree. Preview evidence: Code listing standard/showcase detail examples, plus an emitted static copy-and-wrap fixture. Include `?surface=cli#component-<slug>` links for changed CLI renderers, and the live playground when an interactive adapter flow changes.
...
Effort `refine-2f-69ea5e`, branch `agent/refine-2f-69ea5e`, worktree `/Users/jack/Sites/discern-design-system.worktrees/refine-2f-69ea5e`, based on `c24fe8db6a79`. The landed 1B brief and exact JSON-encoded CopyButton contract were verified before starting.

CodeListing composes CopyButton with the raw source authority, optionally omitted with `copyable={false}`. Both CodeListing and CodeBlock offer authored `wrap`; omission retains horizontal scrolling. This CSS-only viewing preference works without hydration and has no CLI byte effect. Both listing variants share the semantic renderer. Line splitting no longer trims trailing whitespace, LF separators stay in text nodes, empty logical rows have visual height, and gutters are excluded from selection/accessibility text. Long context labels wrap, default browser figure side margins are reset locally, and highlights retain a border witness in forced colours.

The source projection regression failed on the original renderer's fabricated empty-source space. Its parameterized inputs cover empty and trailing source in both variants; the browser fixture guards selected text and exact clipboard transport through independently nested listing instances. This is an owned source-renderer guard, not a claim to audit every source transformation in the package. Terminal formatting and consumer-defined source transformations are outside that scope.

Focused verification covers 320, 390, 720, and 1280px in light/dark, normal and reduced motion, exact native Clipboard API round-trips (tabs, leading/trailing spaces, LF/CRLF, long tokens, Unicode, punctuation, and empty source), logical numbering/highlights, manual selection, keyboard scrolling, and focus. Existing CodeBlock/Markdown Unicode projection, terminal exact frames, and the 1B static/live dependency sweep pass; CodeListing automatically joins that sweep. HTML parsing normalizes CRLF in displayed/selected text; copying retains the authored CRLF string. The CLI listing keeps its existing truncation; terminal CodeBlock remains the lossless inspection alternative.

Visual review used [Code listing detail](http://localhost:17726/catalogue/components/code-listing/?view=all), [narrow light postures](http://localhost:17726/catalogue/reviews/components/?group=Editorial&component=code-listing&width=narrow&theme=light), the corresponding dark postures, and [static copy-and-wrap](http://localhost:17726/dist/static-code-listing/). The static showcase was also inspected at a 390px viewport with long filename, long source token, Unicode, caption, and actual Copied feedback. For manual reproduction, build the fixture with `deno run -A tests/fixtures/static-code-listing.tsx "file://$(pwd)/dist/static-code-listing/"`, then serve on the deterministic port and copy a listing into a plain-text editor. Compare with the fixture's `sourceValues`, including its terminal newline and empty case.

The final measured minimal-docs selection is 41,845 bytes against the original 40,930-byte ceiling (915 bytes, 2.2% over). The public CopyButton dependency adds 1,449 bytes; removing decorative window dots and consolidating showcase styling into a local inverse-role scope saves 1,729 bytes against the first implementation. The remaining focused design keeps source fidelity, header context, copy feedback, highlights, and focus. The owner explicitly approved a 41,852-byte ceiling; the final design measures seven bytes lower, so the standard proposal uses the narrower measured 41,845-byte ceiling.

Canonical image regeneration is currently blocked: `deno task catalogue:images --update` requires `darwin/aarch64/25.6.0` but the host reports `darwin/aarch64/27.0.0`. The shared capture contract remains unchanged; no image or manifest facts were hand-edited. Completion, the `_done/` move, and the final green Proof await canonical image generation. The owner must supply the pinned capture environment or arrange a separately owned platform migration.
