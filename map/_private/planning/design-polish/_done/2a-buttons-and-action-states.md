...
Implemented in `agent/refine-2a-c816e5` at `/Users/jack/Sites/discern-design-system.worktrees/refine-2a-c816e5`. Native disabled/ARIA-disabled and pending actions use readable muted ink, sunken fill and dashed edges; pending shows a loading ring in the existing icon slot or side padding, with rotation disabled under reduced motion. Native browser disabling deliberately wins over preserving focus: no hydration or consumer event interception is introduced. Callers own the pending transition, completion announcement, and focus restoration. Unavailable anchors retain link semantics but omit href and leave the tab sequence. Destructive actions keep explicit Delete wording and flat elevation beside the raised primary.

The focused queued browser suite covers caller-rendered idle→busy→idle identity/geometry and Enter/Space repeat activation; every owned action variant and size in the canonical action-layout examples; long German/Japanese and explicit multiline labels at a 320px local width; 16px/24px root text; density 0.5/1/2; touch capability; both themes, forced colours and reduced motion; fallback serif and Darkness/Accent samples; and consumer-sized 7×3 SVGs in Button, IconButton and Icon. The original size roles and centered text-glyph slots remain authoritative. The full configured gate additionally exercises the composed Foundation alignment and control-size override guards. Pure CLI contracts remain unchanged.

Visual review used the existing contact sheet and reel at port 18446, including the narrow dark Accent 255 Button matrix and light monochrome IconButton contact sheet. The translated label wraps between centered leading/trailing slots; busy preserves the complete label and disabled text remains readable. Canonical images are regenerated by the owning command. The matrix is a separate browser-only layout example so existing canonical example identities retain their purpose.

Review URLs:

- <http://localhost:18446/catalogue/components/button/>
- <http://localhost:18446/catalogue/components/icon-button/>
- <http://localhost:18446/catalogue/reviews/components/?component=button&example=action-layout&mode=contact&width=narrow>
- <http://localhost:18446/catalogue/reviews/components/?component=button&example=action-layout&posture=action-matrix-dark&mode=reel&width=narrow&theme=dark&accent=255>
- <http://localhost:18446/catalogue/reviews/components/?component=icon-button&example=action-layout&mode=contact&width=narrow>
