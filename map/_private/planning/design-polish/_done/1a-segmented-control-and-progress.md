...
SegmentedControl and Progress are public on Web and CLI with generated export and example enrollment. Native radios carry form values; caller-owned controlled updates remain optional. Progress shares value normalization and waiting text across surfaces. Meter receives selection guidance only. Catalogue consumers remain in their assigned streams.

Focused browser checks cover keyboard wrap and disabled-option skipping, label activation, native reset and submission, external form association, disabled-group exclusion, and independent repeated canonical examples. Selection layout is checked at 260, 520, and 900 pixel allocations inside a 1280 pixel page, plus a 260 pixel allocation on a 360 pixel page, at density 0.8 and structure 0.35. Forced-colour focus remains visible. Progress checks zero, intermediate, complete, clamped and non-finite values, accessible names/context, and explicit waiting under reduced motion and forced colours. Terminal checks cover Unicode/ASCII, all colour depths, narrow/wide allocations, and deterministic frames.
