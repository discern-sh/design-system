...
Native direct links work in static HTML. Browser Forward navigation can leave focus on the old note and leave a nested scroller at the old position. The selected `article-navigation` script restores focus and uses native nearest scrolling with the actual scroll containers' padding. It adds no hydration, scrollspy, click interception, or history writes.
