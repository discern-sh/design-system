...
- Run the Catalogue at the worktree's deterministic `discern identity --port` port and complete the required visual review at the exact localhost URLs. Leave the preview running after `discern_done` so the project owner can review your changes. Stop the preview before running `discern_accept`, as acceptance will remove the worktree. Preview evidence: Banner/Callout/Empty state contact sheets plus the live loading-to-outcome Reel. Include `?surface=cli#component-<slug>` links for changed CLI renderers, and the live playground when an interactive adapter flow changes.
...
The final composition uses public Stack/Cluster layout and a semantic border role for the Callout witness. The existing Component appearance guard enforces the border-token boundary across every Component stylesheet. The canonical capture platform migration is integrated from main; images are regenerated from the complete merged authored tree using the owning command. No standard or capture guard is weakened.

The preview remains available on port `19218` for owner review after the gate. Acceptance remains a separate owner decision.
