# 2Q — Fit compositions to their available space

A composition preview initially fits the space actually available, with deliberate exact-width inspection when requested.

**Proposal coverage:** P5. **Worktree name:** `refine-2q`. **Relative size:** S. **Programme:** design-polish.

Other streams are in flight. You own `2Q` only; do not launch, dispatch, or supervise the sibling briefs.
...
Once ready, call `discern_start` from the main checkout with the literal name `refine-2q`, then re-root all reads, edits, commands, and discern calls to its returned absolute path. Follow the await tool's start/update hint if applicable. Read the worktree's `AGENTS.md`, `map/00-orientation/design-principles.md`, and this brief there before source work. Verify every anchor against the live tree; 1C intentionally moves some Catalogue anchors.

## Background and outcome

The viewer currently starts from a fixed width preset; a narrow preset can still exceed the inner padded canvas on a mobile viewport. The recipes and selected-width URL machinery already exist.
...
- `catalogue/pages/compositions/page.tsx`
- `catalogue/styles/compositions.css`
- `tests/catalogue_compositions_test.ts`
- `scripts/conformance/catalogue/compositions.ts`

## Deliverables

- **P5.** Add Fit available as the default when no explicit valid width is requested. Base fit on the actual allocated canvas, accounting for padding, sidebar, and container width rather than guessing from viewport size.

- **Exact inspection.** Retain explicit review widths with understandable labels and measured allocation. Preserve valid existing width URLs and provide a predictable reset to Fit. Treat intentional scrolling at an exact large width as an inspection choice, with visible affordance and keyboard reachability.

- **Responsive behaviour.** Keep recipe selection, copy/adoption actions, loading, and focus stable when the canvas resizes or the user switches Fit/exact widths. Use local containment where appropriate and do not change the marketing recipes themselves.

## Ownership and exclusions

You own:

- Composition viewer page/state helpers, compositions.css, and its own tests/browser checks.
- No recipe contents or shared specimen/detail/terminal modules.

Out of scope: Marketing content/pacing (2H), component-detail allocation (2L), a new canvas engine, saved review sets, and global shell padding changes. All proposals deferred to `discern/TODO.md` stay deferred. Do not opportunistically implement a sibling brief.
...
- At a 390px page and smaller supported allocation, default Fit does not exceed its inner canvas or create document overflow.
- Exact width choices remain measurable and URL-reproducible; switching back to Fit responds to container resize.
- Existing recipe routing, keyboard scroll access, and copy/adoption controls retain their behaviour.
...
- Run the Catalogue at the worktree's deterministic `discern identity --port` port and complete the required visual review at the exact localhost URLs. Stop the preview before the final `discern_done` and keep it stopped through `discern_accept`; a landing may remove the worktree. Preview evidence: A composition URL in default Fit and the same recipe at explicit narrow and wide inspection sizes. Include `?surface=cli#component-<slug>` links for changed CLI renderers, and the live playground when an interactive adapter flow changes.
...
- In the final implementation commit, before the final prepare → commit → done sequence, move `map/_private/planning/design-polish/2q-composition-fit-and-width-controls.md` to `map/_private/planning/design-polish/_done/2q-composition-fit-and-width-controls.md`, repair any moved relative links, and update **only this stream's row** in the programme README to the `_done/` path and completed state, preserving every sibling row.
