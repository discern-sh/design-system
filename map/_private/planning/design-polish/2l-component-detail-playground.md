...
- Run the Catalogue at the worktree's deterministic `discern identity --port` port and complete the required visual review at the exact localhost URLs. Stop the preview before the final `discern_done` and keep it stopped through `discern_accept`; a landing may remove the worktree. Preview evidence: Representative simple/content/structured detail URLs and the copied, type-checked consumer snippets. Include `?surface=cli#component-<slug>` links for changed CLI renderers, and the live playground when an interactive adapter flow changes.
- **Landing rule chosen by the owner:** use discern grants. After green `discern_done`, call `discern_accept` without inventing confirmation. A task grant recorded at the desk covers this brief and any later green commit this agent submits; `discern_accept` checks it at the landing boundary. Without a grant, the verb records the submission and refuses the landing, so report the exact Proof, branch, worktree, preview evidence, and any owner decision, then stop. Never push or publish. A grant does not waive standards or an unmet checkpoint variance.
