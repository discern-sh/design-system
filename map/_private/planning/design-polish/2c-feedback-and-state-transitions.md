# 2C — Clarify feedback and state transitions

People understand what a notice means, what an empty state offers, and how an operation moves from waiting to its outcome.

**Proposal coverage:** D3, D4, D5. **Worktree name:** `refine-2c`. **Relative size:** M. **Programme:** design-polish.

Other streams are in flight. You own `2C` only; do not launch, dispatch, or supervise the sibling briefs.
...
Once ready, call `discern_start` from the main checkout with the literal name `refine-2c`, then re-root all reads, edits, commands, and discern calls to its returned absolute path. Follow the await tool's start/update hint if applicable. Read the worktree's `AGENTS.md`, `map/00-orientation/design-principles.md`, and this brief there before source work. Verify every anchor against the live tree; 1C intentionally moves some Catalogue anchors.

## Background and outcome

Progress is the only new dependency. Banner, Callout, and Empty state already exist; strengthen their semantic structure and the composed loading/result experience using the current foundation roles.
...
- `src/components/feedback/banner/`
- `src/components/feedback/empty-state/`
- `src/components/editorial/callout/`
- `src/components/feedback/meter/`
- `catalogue/review-postures.ts`

## Deliverables

- **D3.** Give informational, warning, and critical notices an intelligible heading/body/action structure and unmistakable state wording or named visible glyph. Tune icon/title/body/action alignment for long messages. Preserve distinctions without relying solely on paint.

- **D4.** Provide distinct generic examples for first use, no search results, unavailable content, and recoverable failure, each with a plausible next action or explanation. Keep the public EmptyState API small and avoid pretending every empty region is an error.

- **D5.** Compose loading→success and loading→failure examples using public Progress where appropriate. Preserve region identity, useful context, and focus; avoid abrupt size changes and redundant announcements. Watch transitions at production speed and with motion reduced. The consumer remains the state-machine owner.

## Ownership and exclusions

You own:

- Banner, Empty state, and editorial Callout folders; bounded feedback transition fixtures inside their examples.
- New tests/feedback_state_transitions_test.tsx and their authored conformance/review postures.

Out of scope: D1 dialog scrolling and D2 toast timing, modifications to Progress internals, global motion tokens, a new application state framework, and editorial article rhythm outside Callout. All proposals deferred to `discern/TODO.md` stay deferred. Do not opportunistically implement a sibling brief.
...
- Every tone has a visible non-colour witness and clear long-message hierarchy in both themes and forced colours.
- The four empty-state examples teach different situations with actionable, generic content.
- State changes retain useful context and keyboard focus, communicate the outcome once, and remain complete without animation.
...
- Run the Catalogue at the worktree's deterministic `discern identity --port` port and complete the required visual review at the exact localhost URLs. Stop the preview before the final `discern_done` and keep it stopped through `discern_accept`; a landing may remove the worktree. Preview evidence: Banner/Callout/Empty state contact sheets plus the live loading-to-outcome Reel. Include `?surface=cli#component-<slug>` links for changed CLI renderers, and the live playground when an interactive adapter flow changes.
