...
The Builder architecture stream can then run alongside Catalogue page waves 4–5 and the later Browser component polish programme because it owns only `catalogue/builder/**` and Builder-specific tests. Builder's final integration stream waits for `catalogue-ux/_done/5a-integrated-visual-qa.md`, `browser-polish/_done/3a-integrated-browser-polish.md`, and its own wave 2. It then composes the final shared search, imagery, Appearance, interaction, motion, and local-responsive authorities before the last proof.
...
| Appearance         | Editor focus/selection/insertion chrome uses stable editor-only semantics. Preview Theme/accent uses the shared state/control decision from Catalogue 5A plus the semantic-safety result of Browser polish 1A/3A; the Builder does not recreate or assume the old primary accent slider. |
| Browser polish     | Public Components keep the final interaction, motion, local-responsive, semantic-colour, hierarchy, and elevation contracts proven by Browser polish 3A. Builder chrome may compose those Components but cannot fork or counter-style their public visual state.                         |
...
| 3A  | [Run the integrated Builder authoring and browser hardening pass](3a-integrated-builder-qa.md)                   | One adversarial browser worktree; read-only audits may fan out | Builder 2A–2D, Catalogue 5A, and Browser polish 3A have landed | Final implementation stream |
...
Wave 3 composes the final workspace and audits the real human journeys after the public browser Component grammar is settled. It is the sole owner of cross-feature Builder conformance orchestration and present-tense Builder map updates. It consumes browser polish rather than counter-styling public Components, and does not reopen the file-management strategy deferred to TODO.
...
- editor chrome still inheriting the consumer accent, a duplicate accent slider ignoring the landed Appearance decision, or Builder CSS counter-styling public Component interaction/motion/responsive states;
...
After Browser polish 3A and then Builder 3A land and pass adversarial review, the Interface Builder is ready for sustained owner use. Named drafts and wider file management remain a deliberate follow-up rather than hidden scope in this programme.
