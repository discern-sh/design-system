...
`deno task catalogue:images --update` builds the real capture projection, derives each entry's rendering-graph fingerprint, and launches the pinned Chromium only for entries whose source, contract, or committed artifact is no longer reusable. A source-current run performs no browser work. `--verify` is wholly source-and-artifact based: it builds the same projection, proves every graph fingerprint, manifest entry, logical and physical dimension, density, canonical PNG shape, content hash, stale file, and orphan without launching Chromium. Browser raster bytes are not a semantic oracle; capture readiness and subject-and-paint geometry constrain a fresh update, while browser conformance and human review judge visual output. [ADR 0041](../_adr/0041-verify-component-images-from-sources-and-artifacts.md) owns that boundary.
