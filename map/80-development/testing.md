...
`deno task catalogue:images --update` is the explicit full-population browser effect. Its `--verify` posture is input-aware: it validates every committed manifest entry and PNG without recapture, then uses the real renderer for a fixed first/middle/last repeat witness. The witness must match exact committed geometry and preserve readiness, font, animation, network, and console invariants; subpixel antialiasing differences are non-semantic and browser rerenders do not promise byte identity. `--update` applies the same judgment when writing: a capture inside the declared raster tolerance keeps the committed file, so only real visual changes reach the working tree. [ADR 0034](../_adr/0034-commit-pinned-png-component-example-images.md) owns that boundary.
