...
**Fix.** Run `deno task catalogue:images --update` and commit the manifest beside the bump. Expect the `sourceHash` line alone to move: the update keeps any image whose capture differs only within the raster tolerance, and names each one it retained. A PNG that does change is a real visual change and deserves review. Budget about sixteen minutes for the recapture — it walks the whole corpus in a real browser, and why it costs that much is an open item in [`discern/TODO.md`](../../discern/TODO.md).
