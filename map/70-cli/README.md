...
The rendered module convention is visible in [`badge.cli.ts`](../../src/components/display/badge/badge.cli.ts): a default pure renderer, a `<Pascal>CliProps` type, and `cliExamples`. Badge's web and terminal renderers both import [`BadgeTone`](../../src/components/display/badge/badge.types.ts), so React does not own their shared vocabulary. Workflow and Agents follow the same rule for every closed status, tone, direction, and variant in a framework-neutral `<slug>.types.ts`; CLI-only item shapes use strings rather than `ReactNode`. `deno task catalogue:cli <slug>` loads the generated module path and prints all named example states.
...
[`tests/cli/`](../../tests/cli/) supplies reusable exact-frame helpers and covers every foundation module, capability degradation, all triangle phases and states, Token enrollment, the two-way Component contract, Badge, and the Workflow and Agents groups. The generated registry contains 109 Components: 30 are rendered and 79 are pending. `[standards.cli_pending]` in [`discern.toml`](../../discern.toml) holds that pending count at a falling ceiling of 79; the standard's measurement imports the same Component discovery authority as Codegen.
