...
Every rendered Component has exact plaintext frames at narrow, standard, and wide widths plus styled expectations at truecolour, ANSI 256, and ANSI 16. The same cases run with no colour and intentional ASCII glyphs. Procedure coverage pins every workflow status and representative spinner phases; Fleet and Worklog pin representative out-and-back beacon phases at every capability level.

## Verification and current state

[`tests/cli/`](../../tests/cli/) supplies reusable exact-frame helpers and covers every foundation module, capability degradation, all triangle phases and states, Token enrollment, the two-way Component contract, Badge, and every rendered Core, Display, Layout, Workflow, and Agents Component at narrow, standard, and wide widths. The generated registry contains 109 Components: 51 are rendered, Theme switcher is exempt, and 57 are pending in the remaining Groups. `[standards.cli_pending]` in [`discern.toml`](../../discern.toml) holds that pending count at a falling ceiling of 57; the standard's measurement imports the same Component discovery authority as Codegen.
