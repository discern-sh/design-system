...
**Authoring.** A designer-developer edits Tokens in [`tokens.ts`](../../src/tokens/tokens.ts) or a Component folder under [`src/components/`](../../src/components/). Each Component's `*.meta.ts` declares its name, slug, Group, ordering, purpose memberships, usage guidance, accessibility notes, any selection-scoped browser behavior, a required rendered or reasoned-exempt CLI stance, and one ordered framework-neutral `componentExampleVocabulary`. Its examples module binds every Web-capable entry to a bounded React renderer. A rendered stance adds a React-free `*.cli.ts` that binds every CLI-capable entry to pure renderer inputs; other vocabulary shared with `*.tsx` lives in a neutral sibling module.
...
**Presentation.** The Catalogue ([`catalogue/`](../../catalogue/)) is the local web component browser: `deno task serve` builds the full Runtime plus the example registry and serves it for human review. It also renders every terminal-foundation registry entry as a searchable projected sheet, including live reduced-motion-safe spinner review. The CLI Catalogue (`deno task catalogue:cli`) prints every rendered Component example, every reasoned exemption, and those same Terminal motifs and Narration lines foundations; a Group, Component slug, or foundation ID narrows it. The CLI Playground (`deno task playground:cli`) is the live terminal counterpart: named journeys exercise every high-level interactive API and browse the generated Component inventory in a real terminal.
