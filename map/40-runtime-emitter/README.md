...
[Static copy actions](static-copy.md) describes exact data transport, script adoption, truthful feedback, and the shared static/live lifecycle. [Static theme preference](static-theme.md) describes the two Theme toggle modes, who owns the first paint and the storage key, and how one activation is never handled twice. [Static docs drawer](static-drawer.md) describes the Docs layout drawer: the toggle contract a consumer renders, the stylesheet-owned breakpoint the behaviour reads, and the first-paint marker. [Static search palette](static-search.md) describes the palette's opener contract, what the behaviour owns and what the consumer's script keeps, and the drawer hand-off.
