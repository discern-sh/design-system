...
The Field/Accent appearance graph, its static and live projections, symmetric browser scopes, and the Root-scoped foundation and utility CSS.

## Field authority and poles

[`field.ts`](../../src/tokens/field.ts) owns paper and ink as OKLab pigments, the bounded darkness, structure, emphasis, and density axes, and one ordered law for each non-series colour role. Every law explicitly carries either its Accent projection or the fact that it remains Field-derived. `evaluateAppearance(appearance, point)` evaluates Field or Accent to deterministic `oklch()` strings; `evaluateField(point)` remains the achromatic compatibility entry point. [`tokens.ts`](../../src/tokens/tokens.ts) evaluates Field at darkness `0` and `1` into the existing `ThemeToken` pair shape, while [`field-css.ts`](../../src/tokens/field-css.ts) walks the same nodes for live browser calculations. Generated pole pairs are static fallback, not another value authority.

Canvas follows darkness and active pigment polarity changes at the `0.179` relative-luminance crossover. Structure scales structural roles, Emphasis scales state and chromatic strength within their admitted bounds, and Density applies only when the numeric four-pixel spacing fact is projected. In Field, `--discern-color-action` is full active ink; in Accent it is the strong selected hue. `--discern-color-on-action` is the opposite Field pigment in both, so the pair always inverts. The hard action shadow retains a structural response while staying at least `0.08` OKLab from fill and canvas in the admission sweep. Raised surface and Avatar identity-fill bases are composited once and emitted opaque. Sunken surfaces, washes, borders, overlays, and decorative Avatar highlight may retain alpha where their owned backdrop is part of the contract.

The Runtime emits Field when its `theme` option is absent or `"none"`. [`theme/blue.ts`](../../src/theme/blue.ts) keeps the opt-in `"blue"` selection and public token shapes as the generated Accent projection at hue `255`; no Blue value table exists. Its action pair therefore uses the same strong-fill/opposite-pigment law as every Accent hue. Consumer role presets remain public Token overrides, never Component CSS.

## Browser projection and authored points

The Token layer registers `--discern-darkness`, `--discern-structure`, `--discern-emphasis`, `--discern-density`, and `--discern-accent-hue` as inherited numeric properties. Inside one feature query, the CSS backend binds repeated expression subtrees once and derives every enrolled role with `calc()`, `min()`, `max()`, `clamp()`, `abs()`, `round()`, `oklch()`, and opaque compositing where required. Series roles remain static. Browser conformance compares every computed Field role and every role in nested Field/Accent scopes with TypeScript within `0.006` OKLab, checks the static pole route independently, and verifies inherited or locally overridden axes and hue.

Consumers select the atomic scope surface with Runtime option `appearanceScopes: true`. The public attribute is `data-discern-appearance="field|accent"`; an Accent scope supplies or inherits `--discern-accent-hue`. All selectors are zero-specificity and contained beneath `[data-discern-root]`. The contract supports Field → Accent → Field, Accent → Field → Accent, and Accent(hue A) → Accent(hue B) nesting. A colour-only scope leaves Darkness, Structure, Emphasis, and Density untouched; setting an axis on the scope changes that coordinate for its descendants through ordinary inheritance.

```html
<main data-discern-root data-discern-appearance="field">
  <section
    data-discern-appearance="accent"
    style="--discern-accent-hue: 255"
  >
    <aside data-discern-appearance="field">Achromatic again</aside>
  </section>
</main>
```
...
Accent accepts every finite hue from `0` through `360`; `360` aliases `0`, fractional values are valid, and invalid or non-finite values throw. Named colours are conveniences rather than an admission list. Success remains centred at hue `152`, warning at `74–82`, and danger at `28`; polarity-relative strength envelopes keep each family recognisable and at least `0.08` OKLab from a coincident Accent and from the other semantic families. Meaning still requires the existing non-colour witness.

[`appearance-admission.ts`](../../src/tokens/appearance-admission.ts) is the package-level proof. It evaluates Field and the complete integer Accent circle, adds fractional seam and semantic-neighbour hues, and visits both poles, the signed 0A postures, polarity neighbours, and low/high Emphasis and Structure. Its 190,000-plus checks retain primary, muted, faint, action, inverse, focus, semantic, fixed-series, and opaque-surface floors and add hard-shadow separation and identity-fill ownership. [`appearance_test.ts`](../../tests/appearance_test.ts) binds this public proof; browser conformance binds the live and nested CSS projections. The current Catalogue picker still exposes its pre-integration named controls; its later integration consumes this authority rather than defining the supported hue domain.
...
Appearance identity is a pure explicit input shared with the browser vocabulary: Field, or Accent plus a hue. [`cli/theme.ts`](../../src/cli/theme.ts) currently evaluates Field at its two poles and consumes already-composited opaque colours; terminal propagation of the Accent input is the next terminal workstream. The terminal never reads browser state or ambient appearance. Existing type roles carry the Field emphasis bands: faint and annotation are dim, body is normal, and strong/display are bold. Series stay package-authored, while every derived truecolour value continues through the shared ANSI 256 and ANSI 16 quantisation proofs.
