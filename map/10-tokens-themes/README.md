...
Appearance identity is a pure explicit input shared with the browser vocabulary: Field, or Accent plus a hue. [`cli/theme.ts`](../../src/cli/theme.ts) evaluates the selected Appearance at the explicit light or dark pole, consumes its already-composited opaque colours, and projects the result through the shared truecolour, ANSI 256, and ANSI 16 authorities. Omission remains the cached monochrome Field default. [`presenter.ts`](../../src/cli/presenter.ts) binds ground and Appearance independently, while `with()` and per-render props can restore Field or replace an Accent hue for one subtree without ambient state. Existing type roles carry the Field emphasis bands; series stay on their package-authored palette. Generated population conformance requires every rendered Component to accept and honour the shared presentation contract across both grounds, every colour depth, and Unicode/ASCII.
