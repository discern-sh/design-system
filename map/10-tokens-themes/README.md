...
The Catalogue exposes only the named Appearance presets in [`appearance-options.ts`](../../catalogue/shell/appearance-options.ts). Options are a union: hue-backed presets move the single public accent hue, while a preset-backed option carries complete role-token overrides applied per resolved theme through one shared style authority ([ADR 0039](../_adr/0039-admit-role-override-appearance-presets.md)). No preset-backed option is authored yet; synthetic presets in the test prove the seam. Each option is admitted after the proof holds in both Themes: accent text and focus contrast, accent-versus-semantic and pairwise semantic distinction, ink/muted/faint legibility on canvas, the inverse pair, and categorical-series visibility and pairwise distinction. Translucent `oklch()` values are composited over the option's opaque canvas in gamma sRGB before measurement, so alpha-derived roles prove real painted contrast. [`catalogue_appearance_options_test.ts`](../../tests/catalogue_appearance_options_test.ts) and browser conformance bind that contract. The public low-level accent hue remains available to coordinated consumer Themes, but an arbitrary hue is not by itself a promise that the unchanged semantic roles are safe.
