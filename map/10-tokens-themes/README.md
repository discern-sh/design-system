...
The optional font asset binds the interface OpenType bundle to `Discern Inter UI` and the marketing headline bundle to `Discern Inter Marketing` through separate `@font-face` descriptors. The marketing role and weight are independent of the serif display, body and UI roles; consumers can replace them locally for branding. Font-specific feature tags never travel through the inherited `font-feature-settings` property or a public Token: the same tag can select unrelated glyphs in another family, as Iowan Old Style's `salt` long-s demonstrates. The system-font UI fallback therefore uses that face's defaults rather than pretending Inter's alternates are portable.
...
Static material shade and highlight follow Structure; optional ambient illumination follows Emphasis and takes a restrained hue only under the existing Accent projection. These decorative roles are authored in [appearance.ts](../../src/tokens/appearance.ts), alongside the established surface/elevation laws. [Material guidance](../20-components/material-presence.md) explains local selection, neutral scopes and excess.

## Appearance safety

Accent accepts every finite hue from `0` through `360`; `360` aliases `0`, fractional values are valid, and invalid or non-finite values throw. Named colours are Catalogue conveniences rather than an admission list. Success remains centred at hue `152`, warning at `74–82`, and danger at `28`; polarity-relative strength envelopes keep each family recognisable and at least `0.08` OKLab from a coincident Accent and from the other semantic families. Meaning still requires the existing non-colour witness. Semantic markers and the hard action preserve the chroma needed for distinction at low Emphasis, while washes can fade. Monochrome markers reserve separation near the crossover; semantic/deep text and focus retain their readable ink rungs. Chromatic semantic text approaches neutral contrast at its actual tinted crossover, including a taper in chroma so gamut clipping does not compromise legibility.

[`appearance-admission.ts`](../../src/tokens/appearance-admission.ts) is the package-level proof. It evaluates monochrome and the complete integer Accent circle, adds fractional seam and semantic-neighbour hues, and visits both poles, the signed 0A postures, polarity neighbours, both Emphasis bounds, low/high Structure, poles at twelve hues with the ink tinted, the paper tinted, and both tinted, tinted crossover neighbours, and a half-tinted dark posture. It retains primary, muted, faint, action, inverse, focus, semantic, fixed-series, and opaque-surface floors, checks semantic text on canvas and its composited wash, adds hard-shadow separation and identity-fill ownership, and sweeps both pigments across the hue circle at four tint strengths for sRGB gamut. [`appearance_test.ts`](../../tests/appearance_test.ts) binds this public proof; browser conformance binds the live and nested CSS projections. The Catalogue consumes the same numeric hue and axes for its Web and CLI projections; named choices remain conveniences over that number rather than another admission authority.

Small text needs `4.5:1`, so at the light and dark poles every rung holds a text-grade floor on each neutral text surface — canvas, raised surface, and sunken wash — under any paper or ink tint (`APPEARANCE_POLE_INK_CONTRAST_FLOORS`). Between the poles the ladder compresses toward the polarity crossover, where even primary ink nears `4.5:1`, and the general rung floors apply. The light-pole faint law uses `0.58` alpha: `0.55` held on white but fell to `4.35:1` on the sunken wash under a full ink tint, and the `0.50` eye-led value from the exploratory findings composites to only `3.94:1` on white. The same contextual proof keeps inverse roles stable light-on-dark: flipping them with active polarity at the dark pole turns existing inverse scopes into white surfaces carrying white canvas-oriented roles.
